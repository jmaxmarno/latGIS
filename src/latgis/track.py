...
class TrackData:
    ...
    def add_track(self, objectLocation: ItemLocation) -> None:
        ...
        """
        new_row = DataFrame([{TrackData.TRACK_ID_STRING: objectLocation.getObjectID(),
                              TrackData.OBJECT_DATA_STRING: objectLocation}])
        if self.__trackDataFrame.empty:
            self.__trackDataFrame = new_row
        else:
            self.__trackDataFrame = pd.concat([self.__trackDataFrame, new_row],
                                              ignore_index=True)
